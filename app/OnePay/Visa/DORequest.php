...
class DORequest
{
    ...
    public static function sendDORequest(Request $request)
    {
        ...
        $payment_data['vpc_Merchant'] = config('one_pay.international.merchant_id');
        $payment_data['vpc_AccessCode'] = config('one_pay.international.access_code');
        $payment_data['vpc_ReturnURL'] = config('one_pay.international.return_url');
        $payment_data['vpc_Command'] = "pay";
        $payment_data['vpc_Currency'] = "VND";
        $payment_data['vpc_Version'] = "2";
        $payment_data['vpc_Locale'] = "en";
        ...
        SaveOrder::whenCreate($request, 'VISA', $payment_transaction_id);
        return config('one_pay.international.url') . '?' . $vpcURL;
    }
}
